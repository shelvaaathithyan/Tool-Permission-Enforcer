    ...
    <div>
      <div className="panel" style={{ position: 'relative', minHeight: '200px' }}>
        <div className="panel-header">
          ...
          <button className="btn btn-secondary btn-sm" onClick={fetchRequests} disabled={loading}>Refresh</button>
        </div>
        ...
        {error ? (
          ...
          <>
              ...
              <table>
                ...
                <tbody>
                  {!loading && requests.length === 0 ? (
                    <tr><td colSpan="6" className="empty-state">No signup requests found.</td></tr>
                  ) : (
                    requests.map(req => (
                      <tr key={req.id}>
                        <td>{req.name}</td>
                        ...
                        <td>
                          {req.status === 'PENDING' && (
                            <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                              ...
                              <button disabled={actionLoading} onClick={() => handleApprove(req.id, req.requested_role === 'STAFF' ? 'MANAGER' : 'STAFF')} className="btn btn-secondary btn-sm">
            ...
            {loading && <PageLoader overlay={true} message="Loading requests..." />}
          </>
        )}
      </div>
    </div>
